...
Directory _tempDir() =>
    Directory.systemTemp.createTempSync('mumukelivo_skill_test_');
...
void main() {
  ...
  group('buildSkillSystemText (pure, S1/P4)', () {
    ...
    test(
      'wraps each skill and adds the follow-instructions meta instruction',
      () {
    ...
    test(
      'escapes XML-significant characters so injected text cannot break out',
      () {
    ...
    test(
      'truncates content past the per-skill cap and bounds the total block',
      () {
    ...
    test(
      'bounds the total block when multiple large skills exceed the cap',
      () {
        final text = buildSkillSystemText(<Skill>[
  ...
  group('SkillFileStorage incremental I/O (P1)', () {
    ...
    test(
      'unsafe id is hashed into a safe filename contained in the dir',
      () async {
  ...
  group('SkillProvider incremental persist (P1)', () {
    ...
    test('updateSkill rewrites the same file in place', () async {
      ...
      await provider.updateSkill(
        provider.getById(id)!.copyWith(content: 'new'),
      );
  ...
  group('SkillProvider graceful degradation (G2)', () {
    test(
      'broken store file never throws and keeps the provider usable',
      () async {
        await File(
          '${tempDir.path}/broken.json',
        ...
        expect(
          provider.resolveActiveSkills(explicitSkillIds: const <String>[]),
          isEmpty,
        );
    ...
    test(
      'explicitly bound skill is injected even when globally disabled',
      () async {
    ...
    test(
      'globally disabled skill is not keyword-triggered implicitly',
      () async {
        await storage.save(
          _skill(
            id: 's1',
  ...
  group('Zip import safety (S2)', () {
    test(
      'per-entry size guard skips oversized files but keeps valid ones',
      () async {
    ...
    test(
      'imports only the primary SKILL.md from a multi-file package',
      () async {
        ...
        final zip = _zipWithEntries(<String, List<int>>{
          ...
          'references/story-bible-template.md': utf8.encode(
            '# Bible\ntemplate',
          ),
    ...
    test(
      'loose archive without a primary SKILL.md imports all files',
      () async {
        final zip = _zipWithEntries(<String, List<int>>{
          'a.md': utf8.encode('# A\none'),
          'b.md': utf8.encode('# B\ntwo'),
  ...
  group('Secret detection (S4)', () {
    test('flags common secret patterns', () {
      ...
      expect(
        skillContentLooksSecret('token: AIzaSyA1234567890abcdefghijklmnop'),
        isTrue,
      );
