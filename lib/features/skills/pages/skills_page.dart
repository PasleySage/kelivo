...
class _SkillsPageState extends State<SkillsPage> {
  ...
  Future<void> _deleteSkill(Skill skill) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(l10n.skillsDeleteConfirmMessage(skill.name)),
        actions: [
          TextButton(
          ...
          TextButton(
            ...
            child: Text(l10n.skillsDeleteAction),
          ),
        ],
      ),
    );
  ...
  void _toggleSelectAll(List<Skill> skills) {
    setState(() {
      if (_selectedIds.length == skills.length) {
        ...
        _selectedIds
          ..clear()
          ..addAll(skills.map((s) => s.id));
      }
    });
  }

  Future<void> _deleteSelected(List<Skill> skills) async {
    final l10n = AppLocalizations.of(context)!;
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        title: Text(l10n.storageSpaceDeleteConfirmTitle),
        content: Text(l10n.skillsDeleteSelectedConfirm(count)),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(ctx).pop(false),
            child: Text(MaterialLocalizations.of(ctx).cancelButtonLabel),
    ...
    for (final s in skills) {
      if (_selectedIds.contains(s.id)) {
        await provider.deleteSkill(s.id);
      }
    }
    _exitSelectionMode();
  }
        ...
        actions: _selectionMode
            ? [
                if (skills.isNotEmpty)
                  TextButton(
                    onPressed: () => _toggleSelectAll(skills),
                    child: Text(
                      _selectedIds.length == skills.length
                          ? l10n.skillsSelectNone
                ...
                TextButton(
                  onPressed: _selectedIds.isEmpty
                      ? null
                      : () => _deleteSelected(skills),
                  child: Text(
                    l10n.skillsDeleteSelected(_selectedIds.length),
                    style: TextStyle(
                ...
                IconButton(
                  icon: Icon(Lucide.X, color: cs.onSurface, size: 20),
            ...
            : [
                Tooltip(
                  message: l10n.skillsMultiSelect,
                  child: IosIconButton(
                    icon: Lucide.CheckSquare,
                    color: cs.onSurface,
                    size: 20,
                    minSize: 44,
                    semanticLabel: l10n.skillsMultiSelect,
                ...
                if (_selectionMode) {
                  return Padding(
                    ...
                    child: _SkillTile(
                      ...
                      onSelect: () => _toggleSelect(skill.id),
                      onTap: () => _toggleSelect(skill.id),
                      onDelete: () {},
                      onEnabledChanged: (_) {},
                    ),
                  );
            ...
            children: [
              ...
              if (selectionMode)
                ...
                IconButton(
                  ...
                  tooltip: l10n.skillsDeleteAction,
                  onPressed: onDelete,
                ),
