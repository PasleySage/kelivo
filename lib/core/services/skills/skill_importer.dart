...
class SkillImporter {
  ...
  // parser to avoid loading a huge file entirely into memory (#6).
  static const int maxDocBytes = 50 * 1024 * 1024; // 50 MB
  ...
  static List<ImportedSkill> _importManyFromZip(List<int> bytes) {
    ...
    // archive as ONE skill: import only the primary file instead of
    // flattening every companion into its own skill. Loose archives that
    // contain no primary SKILL.md keep the previous "import all supported
    // files" behavior.
    final hasPrimarySkillMd = supported.any(
      (entry) => _isAllowedSkillEntry(entry.name),
    );
    final entries = hasPrimarySkillMd
        ? supported
              .where((entry) => _isAllowedSkillEntry(entry.name))
              .toList(growable: false)
        : supported;
    entries.sort((a, b) {
    ...
    for (final entry in entries.take(maxZipSkills)) {
      try {
        ...
        // S2: per-entry size guard against zip bombs.
        if (entryBytes.length > maxUncompressedEntryBytes) continue;
        totalUncompressed += entryBytes.length;
        if (totalUncompressed > maxUncompressedArchiveBytes) {
          throw const FormatException(
            'Skill archive exceeds the decompression size budget.',
          );
        }
        imported.addAll(
        ...
        rethrow; // budget/format errors must surface, not be swallowed
      } catch (_) {
  ...
  static bool _isSupportedSkillEntry(String rawName) {
    ...
    final parts = normalized
        ...
        .where((part) => part.isNotEmpty)
        .toList(growable: false);
    // S2: cap recursion depth at 3 path segments (≤2 nested folders).
    if (parts.isEmpty || parts.length > 3) return false;
  ...
  static String _extractPdfText(List<int> bytes) {
    if (bytes.length > maxDocBytes) {
      throw const FormatException('PDF is too large to import.');
    }
  ...
  static String _extractDocxText(List<int> bytes) {
    if (bytes.length > maxDocBytes) {
      throw const FormatException('DOCX is too large to import.');
    }
    final archive = ZipDecoder().decodeBytes(bytes, verify: false);
